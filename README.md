...
- **车辆控制**分页:车门/车窗/空调/充电(含电流调节与实时功率)/哨兵/闪灯鸣笛/前后备箱/导航推送/除霜/方向盘与座椅加热/生化模式等白名单指令,滑动开关操作、下发前自动唤醒车辆、指令审计日志、定时哨兵时段;OTA 更新徽标、车身按实车配色渲染、车机定时充电只读展示;指令后端兼容官方 tesla-http-proxy 与托管服务(详见 [docs/CONTROL.md](docs/CONTROL.md))
