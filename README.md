...
<table>
  <tr>
    <td><img src="docs/screenshots/demo-switch.gif" alt="桌面端:翻页动效"></td>
    <td><img src="docs/screenshots/demo-mobile.gif" alt="手机端:滑动选页"></td>
  </tr>
  <tr>
    <td align="center">桌面端 · 翻页动效</td>
    <td align="center">手机端 · 横向滑动选页(气泡跟手,松手吸附)</td>
  </tr>
</table>

![总览页](docs/screenshots/demo-overview.png)

![控制页](docs/screenshots/demo-control.png)

## 超越官方 Tesla App 的重点功能

官方 App 侧重"控制车辆",TESLA Home 侧重"看懂车辆"——以下数据维度官方 App 没有或远不够细:

- **哨兵模式耗电计量**:官方 App 只有哨兵开关,看不到它到底耗了多少电;面板自动推断哨兵时段,按天统计耗电量与电费
- **电池健康度**:用充电历史自校准满电容量,当前值对比历史基准,电池衰减一目了然
- **家充峰谷自动计价**:按地点配置峰/谷电价与时段,自动精确分摊;每次充电的单价、每公里成本、桩端损耗全都有
- **驻车耗电拆分**:哨兵 / 驻车空调 / 休眠掉电分类统计,回答"停一晚上掉了多少电、为什么"
- **充电提醒**:按通勤规律 + 停放耗电预测电量还能撑几天、该在家还是公司充
- **行程海拔曲线**:每趟行程的海拔变化图,上下坡对能耗的影响看得见
- **能耗对标**:实时均耗 Wh/km 与官方标定值同尺对比
- **胎压 / 温度历史曲线**:官方 App 只有当前胎压读数,面板保留四轮胎压与车内外温度的完整历史
- **活动时间线**:行驶 / 充电 / 哨兵 / 驻车双泳道回放,每个事件都带上耗电与电费
- **数据完全自己持有**:全部数据在本地 PostgreSQL,一键整库备份/迁移,不依赖任何厂商云服务
...
- 家充峰谷自动计价:按地点配置峰/谷电价与谷时段,自动按日分段精确分摊计费,手填费用永远优先
- 充电提醒:自动识别家/公司充电地点,按通勤与停放耗电(哨兵单列)预测剩余可撑天数
- 充电桩统计:按地点聚合次数/充电量/损耗/费用
- 电量活动时间线:行驶 / 充电 / 哨兵 / 驻车耗电 双泳道分段
- 活动事件、哨兵时间轴与耗电曲线、平均能耗变化、胎压变化、车内/车外温度、每日行驶里程、充电记录
- 行程轨迹地图:按天分组、可折叠行程列表,展开详情含均速/能耗/电费键值表、海拔变化图与该趟小地图;点地图轨迹联动展开对应行程
- **车辆控制**分页:车门/车窗/空调/充电/哨兵/闪灯鸣笛/前后备箱等白名单指令,滑动开关操作、下发前自动唤醒车辆、指令审计日志、定时哨兵时段;指令后端兼容官方 tesla-http-proxy 与托管服务(详见 [docs/CONTROL.md](docs/CONTROL.md))
...
- 面板**个人中心**:自助完成 Tesla 账号授权(分步可视化指引)、修改密码、管理账号、整库数据备份/迁移
- 支持 iPhone 设备证书(mTLS)免密登录,证书在个人中心自助下载
...
- 充电桩品牌背景 logo:国家电网(en.wikipedia [State Grid Corporation of China logo.svg](https://en.wikipedia.org/wiki/File:State_Grid_Corporation_of_China_logo.svg))、特斯拉(Wikimedia Commons [Tesla Motors Logo.svg](https://commons.wikimedia.org/wiki/File:Tesla_Motors_Logo.svg),PD-textlogo)

## 许可证

本项目采用 [PolyForm Noncommercial 1.0.0](https://polyformproject.org/licenses/noncommercial/1.0.0/) 协议(见 [LICENSE](LICENSE)):

- ✅ 允许:个人使用、学习研究、非营利组织/学校/政府机构使用、修改与再分发(需附带协议文本)
- ❌ 禁止:任何商业用途(包括但不限于以盈利为目的的使用、嵌入商业产品、提供付费服务)
